...
const Product = () => {
  ...
  // Only show product if it has a valid image (gallery or thumbnailUrl)
  const hasGallery = product?.gallery && product.gallery.length > 0;
  const hasValidThumb = typeof product?.thumbnailUrl === 'string' && product.thumbnailUrl.trim() !== '';
  if (!hasGallery && !hasValidThumb) return null;
